...
import { SortColumn, SortDirection } from '../util/kecamatan-sortable.directive';
import { ListKecamatan } from '../util/list-kecamatan';

interface SearchResult {
  itemList: ListKecamatan[];
  total: number;
}
...
function sort(itemList: ListKecamatan[], column: SortColumn, direction: string): ListKecamatan[] {
  if (direction === '' || column === '') {
    return itemList;
  } else {
...
export class ListKecamatanService {
  private _loading$ = new BehaviorSubject<boolean>(true);
  private _search$ = new Subject<void>();
  private _itemList$ = new BehaviorSubject<ListKecamatan[]>([]);
  ...
  constructor(private pipe: DecimalPipe, private storage: StorageMap) {
    ...
    this.storage.get(StorageConstants.SETTINGS_KECAMATAN).subscribe((listKecamatan: ListKecamatan[]) => {
        this._items = listKecamatan;
