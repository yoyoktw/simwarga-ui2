...
import { SortColumn, SortDirection } from '../util/kabkota-sortable.directive';

interface SearchResult {
  itemList: ListKabkota[];
  total: number;
}
...
function sort(itemList: ListKabkota[], column: SortColumn, direction: string): ListKabkota[] {
  if (direction === '' || column === '') {
    return itemList;
  } else {
...
export class ListKabkotaService {
  private _loading$ = new BehaviorSubject<boolean>(true);
  private _search$ = new Subject<void>();
  private _itemList$ = new BehaviorSubject<ListKabkota[]>([]);
